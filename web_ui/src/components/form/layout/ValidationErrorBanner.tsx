...
export function ValidationErrorBanner({ message, onDismiss }: ValidationErrorBannerProps) {
  return (
    <div className="rounded-xl border border-destructive/40 bg-destructive/5 p-4 animate-slide-in-up">
      <div className="flex items-start gap-3">
        <div className="icon-chip icon-chip-rose flex-shrink-0 mt-0.5">
          <AlertTriangle className="w-4 h-4" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-mono-label text-destructive mb-1">Validation error</p>
          <p className="text-sm text-destructive/90 whitespace-pre-line leading-relaxed">{message}</p>
        </div>
        <button
          type="button"
          onClick={onDismiss}
