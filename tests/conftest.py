...
def sample_template_structure(temp_dir):
    ...
    (common_dir / "pyproject.toml.j2").write_text("[project]\nname = \"{{ project_name }}\"")
    (common_dir / "gitignore").write_text("*.pyc\n__pycache__/")
    (common_dir / "Makefile").write_text("install:\n\tpip install -e .")
    
    (common_dir / "configs").mkdir()
    (common_dir / "configs" / "config.yaml.j2").write_text("project:\n  name: {{ project_name }}")
    
    # Scikit-learn specific files
    (sklearn_dir / "src" / "train.py.j2").write_text("# {{ framework }} training script")
    (sklearn_dir / "src" / "inference.py.j2").write_text("# {{ framework }} inference script")
    (sklearn_dir / "src" / "models" / "__init__.py").write_text("")
    (sklearn_dir / "src" / "models" / "classification_model.py.j2").write_text("# Classification model")
    (sklearn_dir / "src" / "models" / "regression_model.py.j2").write_text("# Regression model")
    (sklearn_dir / "src" / "models" / "timeseries_model.py.j2").write_text("# Time series model")
    ...
    (sklearn_dir / "src" / "features" / "feature_engineering.py.j2").write_text("# Feature engineering")
    
    # PyTorch specific files
    (pytorch_dir / "src" / "train.py.j2").write_text("# {{ framework }} training script")
    (pytorch_dir / "src" / "inference.py.j2").write_text("# {{ framework }} inference script")
    (pytorch_dir / "src" / "models" / "__init__.py").write_text("")
    (pytorch_dir / "src" / "models" / "classification_model.py.j2").write_text("# Classification model")
    (pytorch_dir / "src" / "models" / "regression_model.py.j2").write_text("# Regression model")
    (pytorch_dir / "src" / "models" / "timeseries_model.py.j2").write_text("# Time series model")
    ...
    (pytorch_dir / "src" / "utils" / "training_utils.py.j2").write_text("# Training utilities")
    
    # TensorFlow specific files
    (tensorflow_dir / "src" / "train.py.j2").write_text("# {{ framework }} training script")
    (tensorflow_dir / "src" / "inference.py.j2").write_text("# {{ framework }} inference script")
    (tensorflow_dir / "src" / "models" / "__init__.py").write_text("")
    (tensorflow_dir / "src" / "models" / "classification_model.py.j2").write_text("# Classification model")
    (tensorflow_dir / "src" / "models" / "regression_model.py.j2").write_text("# Regression model")
    (tensorflow_dir / "src" / "models" / "timeseries_model.py.j2").write_text("# Time series model")
    ...
    (tensorflow_dir / "src" / "utils" / "training_utils.py.j2").write_text("# Training utilities")
